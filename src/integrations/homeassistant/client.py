...
class HomeAssistantClient:
    ...
    async def get_camera_snapshot(self, camera_entity_id: str) -> Optional[bytes]:
        """
        Scarica uno snapshot JPEG dalla camera tramite HA REST API.

        Args:
            camera_entity_id: es. 'camera.ingresso' o 'camera.giardino'

        Returns:
            Immagine in bytes (JPEG) oppure None in caso di errore
        """
        try:
            url = f"{self.settings.ha_url}/api/camera_proxy/{camera_entity_id}"
            async with self.session.get(url) as response:
                if response.status == 200:
                    image_bytes = await response.read()
                    logger.info(f"📸 Snapshot ottenuto da {camera_entity_id} ({len(image_bytes)} bytes)")
                    return image_bytes
                else:
                    logger.error(
                        f"Errore snapshot camera {camera_entity_id}: HTTP {response.status}"
                    )
                    return None
        except Exception as e:
            logger.error(f"Errore get_camera_snapshot({camera_entity_id}): {e}")
            return None

    async def get_camera_entities(self) -> list:
        """
        Restituisce la lista delle entità camera disponibili in HA.

        Returns:
            Lista di dict con 'entity_id', 'friendly_name', 'state'
        """
        try:
            states = await self.get_states()
            cameras = []
            ...
            logger.info(f"📹 Trovate {len(cameras)} camere in HA")
            return cameras
        except Exception as e:
            logger.error(f"Errore get_camera_entities: {e}")
            return []
